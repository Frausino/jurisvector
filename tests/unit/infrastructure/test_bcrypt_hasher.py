...
@pytest.mark.unit
def test_hashing_a_password_returns_bcrypt_formatted_string(hasher: BcryptPasswordHasher) -> None:
    """Hash deve seguir formato `$2b$<rounds>$<22-char-salt><31-char-hash>`."""
...
@pytest.mark.unit
def test_correct_password_verifies_against_its_own_hash(hasher: BcryptPasswordHasher) -> None:
...
@pytest.mark.unit
def test_password_too_short_raises_validation_error(hasher: BcryptPasswordHasher) -> None:
    """Política: senha abaixo de 8 caracteres é rejeitada na origem."""
...
@pytest.mark.unit
def test_malformed_hash_returns_false_instead_of_raising(hasher: BcryptPasswordHasher) -> None:
...
@pytest.mark.unit
def test_two_hashes_of_same_password_differ_due_to_salt(hasher: BcryptPasswordHasher) -> None:
